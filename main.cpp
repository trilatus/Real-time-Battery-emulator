...
public:
    ...
    Sim(Params params_) : params(params_) {
        ams = std::round(params.bat_ah * 3600000);
        soc = params.soc[0];
        load_time = 0;
    }
...
float param_window(auto io, auto &sim) {
    ...
    ImGui::Text("Current: %.2f A", float(load));
    ImGui::Text("Time/fr: %.0f ms", float(sim.params.ms));
    ImGui::Dummy(ImVec2(0, 20));
    ImGui::SliderInt("Load A", &load, 0, 30);
